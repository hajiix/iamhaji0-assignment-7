...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    plt.plot(X, model.predict(X.reshape(-1, 1)), color='orange', label=f"Regression Line: Y = {slope:.2f}X + {intercept:.2f}")
    plt.xlabel("X")
    plt.ylabel("Y")
...
def hypothesis_test():
    ...
    print("Session N value set:", session.get("N"))
    ...
    if test_type == "greater":
        # Proportion of simulated stats greater than or equal to observed stat
        p_value = np.mean(simulated_stats >= observed_stat)
    elif test_type == "less":
        # Proportion of simulated stats less than or equal to observed stat
        p_value = np.mean(simulated_stats <= observed_stat)
    else:
        # Proportion of simulated stats with greater absolute deviation from hypothesized value
        p_value = np.mean(abs(simulated_stats - hypothesized_value) >= abs(observed_stat - hypothesized_value))


    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    # If p_value is very small, set a fun message
    if p_value <= 0.0001:
        fun_message = "Wow! This result is statistically significant at an incredible level! 🎉"
    elif p_value <= 0.01:
        fun_message = "Significant result! Seems like there's a strong effect. 🌟"
    elif p_value <= 0.05:
        fun_message = "Moderately significant - there's some evidence to support the hypothesis! 📊"
    else:
        fun_message = "Not significant - this one might need a closer look. 🤔"
